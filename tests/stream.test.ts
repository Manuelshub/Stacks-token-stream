...
describe("test token streaming contract", () => {
  beforeEach(() => {
    ...
    expect(result.events[0].event).toBe("stx-transfer-event");
  ...
  it("ensures recipient can withdraw tokens over time", () => {
    ...
    expect(withdraw.events[0].event).toBe("stx-transfer-event");
  ...
  it("ensures sender can withdraw excess tokens", () => {
    ...
    expect(refund.events[0].event).toBe("stx-transfer-event");
